...
optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate,weight_decay=5e-7)
scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min',
    factor=0.2, patience=1, threshold=0.1, threshold_mode='abs')
...
def train():
    ...
    for epoch in range(num_epochs):
        ...
        # # early stop
        # if epoch>5:
        #     if(valid_ppl[-1]>valid_ppl[-2] and valid_ppl[-2]>valid_ppl[-3]and valid_ppl[-3]>valid_ppl[-4]):
        #         break

    dataframe = pd.DataFrame({'train_nll':train_nll,'train_ppl':train_ppl,'valid_nll':valid_nll,"valid_ppl":valid_ppl})
    dataframe.to_csv('train_valid_loss_{}_{}_layer1_no_dropout.csv'.format(batch_order,zero_or_former),index=False,sep=',')   
    with open("lstm_832_{}_{}_layer1_no_dropout.pt".format(batch_order,zero_or_former),'wb') as f:
        torch.save(model,f)
